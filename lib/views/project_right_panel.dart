  ...
  Future<void> _loadAll() async {
    ...
    try {
      ...
      if (proyecto != null && auth.user?.role == 'estudiante') {
        ...
        print('[DEBUG FRONTEND] proyecto.es_estudiante_asignado = $flagServidor');
        print('[DEBUG FRONTEND] proyecto.curso_id = ${proyecto?['curso_id']}');
        print('[DEBUG FRONTEND] proyecto.estudiante_id = ${proyecto?['estudiante_id']}');
              ...
              : SingleChildScrollView(
                  child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 12),
                        // Upload drop area or button
                        if (role == 'estudiante' && esEstudianteAsignado) ...[
                        ...
                        // If student but not assigned, show explanatory hint and a quick check button
                        if (role == 'estudiante' && !esEstudianteAsignado) ...[
                          Card(
                              color: AppColors.bg,
                              child: Padding(
                                  ...
                                  child: Column(
                                      ...
                                      children: [
                                        const Text(
                                            'No puedes subir: no estás asignado a este proyecto',
                                            style: TextStyle(
                                                fontWeight: FontWeight.w600)),
                                        const SizedBox(height: 8),
                                        Text(
                                            'Si crees que deberías poder subir, pulsa "Comprobar" para verificar tu asignación al curso/proyecto.'),
                                        const SizedBox(height: 8),
                                        Row(
                                            mainAxisAlignment:
                                                MainAxisAlignment.end,
                                            children: [
                                              TextButton(
                                                  onPressed: () async {
                                                    // quick check: show proyecto fields and enrolled status
                                                    final pid = proyecto?[
                                                        'estudiante_id'];
                                                    final cid =
                                                        proyecto?['curso_id'];
                                                    if (cid == null) {
                                                      ScaffoldMessenger.of(
                                                          ...
                                                          .showSnackBar(SnackBar(
                                                              content: Text(
                                                                  'Proyecto tiene estudiante_id=$pid y no está asociado a curso')));
                                                      return;
                                                    }
                                                    try {
                                                      final lista =
                                                      ...
                                                      ScaffoldMessenger.of(
                                                          ...
                                                          .showSnackBar(SnackBar(
                                                              content: Text(
                                                                  'Inscrito en curso $cid: ${enrolled ? 'sí' : 'no'} — proyecto.estudiante_id=$pid')));
                                                    } catch (e) {
                                                      ScaffoldMessenger.of(
                                                              context)
                                                          .showSnackBar(SnackBar(
                                                              content: Text(
                                                                  'Error comprobando inscripción: $e')));
                                                    }
                                                  },
                                                  child:
